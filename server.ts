// ─────────────────────────────────────────────────────────────────────────────
// Backend web del prototipo (nacido en Google AI Studio, cuando NodeFlow era una
// web). NO lo usa la app de escritorio: esa corre el backend Rust/axum de
// `src-tauri/`, y su build (`npm run build`) sólo compila el frontend con Vite.
// Se conserva a propósito: si volvemos al camino web, se retoma desde acá.
//
// Para correrlo hay que reinstalar sus dependencias (se sacaron del package.json
// para no arrastrar avisos de seguridad al build de escritorio):
//     npm i express @types/express dotenv tsx esbuild
//     npx tsx server.ts        # o: npm run build && node dist/server.cjs
// Está excluido del type-check (tsconfig.json) por ese mismo motivo.
// ─────────────────────────────────────────────────────────────────────────────
