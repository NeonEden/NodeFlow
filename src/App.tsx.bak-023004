...
export default function App() {
  ...
  // Autoguardado continuo del lienzo activo (nodos, aristas y apariencia)
  useEffect(() => {
        ...
        try {
          ...
          if (data.success && Array.isArray(variations) && variations.length > 0) {
            ...
            setNodes((currentNodes) => {
              ...
              return [...currentNodes, ...newCreatedNodes];
            });
          ...
          if (data.success && Array.isArray(variations) && variations.length > 0) {
            ...
            setNodes((currentNodes) => {
              ...
              variations.forEach((v: any, index: number) => {
                ...
                const newId = `node-exp-${Date.now()}-${index}-${Math.floor(Math.random() * 1000)}`;
        ...
        try {
          ...
          if (data.success && Array.isArray(variations) && variations.length > 0) {
            ...
            setNodes((currentNodes) => {
              ...
              const existingChildren = currentNodes.filter((n) => outgoing.some((e) => e.target === n.id));
              ...
              variations.forEach((v: any, index: number) => {
                const newX = parentX + (index - 1) * 320;
                const newY = baseY;
                const newId = `node-crit-${Date.now()}-${index}-${Math.floor(Math.random() * 1000)}`;
          ...
          if (data.success && Array.isArray(variations) && variations.length > 0) {
            ...
            setNodes((currentNodes) => {
              ...
              variations.forEach((v: any, index: number) => {
                const newX = parentX + (index - 1) * 320;
                const newY = baseY;
                const newId = `node-soc-${Date.now()}-${index}-${Math.floor(Math.random() * 1000)}`;
